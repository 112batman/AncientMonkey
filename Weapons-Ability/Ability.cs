...
public class BenjaminEndOfRoundCash : AbilityTemplate
{
    ...
    {
        var towerModel = tower.rootModel.Duplicate().Cast<TowerModel>();
        var ab = Game.instance.model.GetTowerFromId("Benjamin 20").GetBehavior<PerRoundCashBonusTowerModel>()
            .Duplicate();
        towerModel.AddBehavior(ab);
        tower.UpdateRootModel(towerModel);
    }
...
public class BananaStockExchange : AbilityTemplate
{
    ...
    public override void EditTower(Tower tower)
    {
        foreach (var ability in GetContent<AbilityTemplate>().OrderByDescending(c => c.mod == mod))
                ...
                if (ability.stackIndex > 1)
                {
                            ...
                            if (towercreate.towerModel.HasBehavior<AirUnitModel>())
                            {
                                foreach (var attackModel in towercreate.towerModel.GetBehavior<AirUnitModel>()
                                    ...
                                    if (towerModel.GetBehavior<TowerCreateTowerModel>().towerModel
                                        ...
                                        {
                                            towerModel.GetAttackModel().weapons[0].projectile.GetBehavior<CashModel>()
                                                .minimum *= 1.5f;
                                            towerModel.GetAttackModel().weapons[0].projectile.GetBehavior<CashModel>()
                                                .maximum *= 1.5f;
                                        }
                    ...
                    foreach (var attackModel in towerModel.GetDescendants<AttackModel>().ToArray())
                        if (!attackModel.weapons[0].projectile.HasBehavior<SlowModel>())
                            if (attackModel.weapons[0].projectile.HasBehavior<CashModel>())
                            {
                                towerModel.GetAttackModel().weapons[0].projectile.GetBehavior<CashModel>().minimum *=
                                    1.5f;
                                towerModel.GetAttackModel().weapons[0].projectile.GetBehavior<CashModel>().maximum *=
                                    1.5f;
                            }
...
{
    public static List<string> AbilityName = new();
    public static List<string> AbilityImg = new();
    public static List<Sprite> AbilityCustomImg = new();
}